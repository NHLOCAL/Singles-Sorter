...
def scan_dir(dir_path, target_dir, copy_mode=False, abc_sort=False, exist_only=False, singles_folder=True, main_folder_only=False, progress_callback=None):
    ...
    # יצירת רשימה עבור לוג של הקבצים שהועברו או הועתקו
    operating_details = dir_path, target_dir, copy_mode, abc_sort, exist_only, singles_folder, main_folder_only
    log_files = []
    ...
    for file_path, artist in info_list:   
        ...
        if copy_mode and os.path.isdir(target_path):
            try:
                ...
                log_files.append((file_path, target_path))
    ...
    return log_files, operating_details
...
def log_to_file(log_files, operating_details):
    now = datetime.datetime.now()
    log_filename = f"log_{now.strftime('%Y%m%d_%H%M%S')}.json"
    
    log_data = {
        "operation_time": now.strftime('%d/%m/%Y %H:%M:%S'),
        "source_directory": operating_details[0],
        "target_directory": operating_details[1],
        "copy_mode": True if operating_details[2] else False,
        "abc_sort": True if operating_details[3] else False,
        "exist_only": True if operating_details[4] else False,
        "singles_folder": True if operating_details[5] else False,
        "main_folder_only": True if operating_details[6] else False,
        "files": [{"old_path": old_path, "new_path": new_path} for old_path, new_path in log_files]
    }
...
def load_from_log(log_filename):
...
def main():
    ...
    try:
        ...
        log_files, operating_details = scan_dir(dir_path, target_dir, copy_mode, abc_sort, exist_only, singles_folder, main_folder_only)

        # שמירת לוג עם פרטים מלאים + רשימת הקבצים שהשתנו לקובץ
        log_to_file(log_files, operating_details)
