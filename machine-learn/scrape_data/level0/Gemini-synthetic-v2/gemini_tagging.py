...
**מטרה:** ליצור דאטה איכותי ומסומן היטב לאימון מודל זיהוי ישויות (NER) שיזהה את הקטגוריות הרלוונטיות בתוך שמות שירים, תוך הקפדה על דיוק מלא והימנעות מקיטוע מילים או ערבוב קטגוריות.

**קטגוריות לזיהוי:**
1. **`SONG`**: שם השיר המלא (כולל כותרות משנה, עיבודים ותוספות אמנותיות).
2. **`SINGER`**: שם האמן, הלהקה, המקהלה או התזמורת. יש לתייג כל זמר או קבוצה בנפרד, ולהימנע מתוספות שאינן חלק מן השם (ללא תארים).
3. **`GENRE`**: ז'אנר או סגנון מוזיקלי.
4. **`ALBUM`**: שם האלבום, אם מופיע.
5. **`MISC`**: מידע שאינו קשור למוזיקה ואינו מספר רצועה, כגון מקומות, אירועים, או פרטים לא-מוזיקליים אחרים.

**הוראות כלליות:**
- הקפד לאתר את כל הישויות במלואן – אל תחתוך שמות באמצע.
- הימנע משימוש ב-`MISC` עבור מספרי רצועה או אלמנטים טכניים שאינם שם אמיתי.
- ודא שטווחי התווים מדויקים, כך שהישויות כוללות בדיוק את המחרוזת המתאימה להן.
- השתמש בעקביות באותה מתודולוגיה עבור כל השירים.
- תייג כל זמר או מקהלה בנפרד, ללא תוספות תואר ושאינן חלק משמו.

**מבנה הדאטה:**
- מערך JSON.
- כל איבר: [ "<שם השיר>", { "entities": [ [start, end, "TYPE"], ... ] } ]

פועלך יסייע ביצירת דאטה איכותי לצורך אימון מדויק של מודל NER.
""",
)
