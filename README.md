...
ברירת המחדל בממשק וב־CLI היא **העברה**, ללא יצירת תיקיית ״סינגלים״ פנימית.
אפשר להפעיל שמירת עותק ותיקיית ״סינגלים״ בהגדרות המתקדמות; ב־CLI השתמשו
ב־`--copy_mode` וב־`--singles-dir` בהתאמה.

הממשק כולל גם תיקון שמות ותגיות, רשימת זמרים אישית עם ייבוא וייצוא CSV,
מרכז עזרה, מסך ״מה חדש״, אודות התוכנה, טיפים ובדיקת עדכונים שאינה חוסמת את
הפעלת התוכנה. תוצר Windows העצמאי נמצא לאחר הבנייה תחת `build/windows`.
